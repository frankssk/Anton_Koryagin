...
def currency_output(valute_code, valute_convert):
    '''
    ...
    :valute_input Запрашивает количество валюты.
    '''
    ...
    if valute_code not in valute:
        ...
        valute_convert = Decimal(Decimal(valute_convert) * (valute[valute_code][5] / int(valute[valute_code][4])))
        print(f'ID:                {(valute[valute_code])[0]}\n'
              ...
              f'Итог:              {valute_convert.quantize(Decimal("1.00"))} RUB\n'
              f'Дата:              {date}\n'
...
def input_outpat():
    ...
    '''
    valute_inquiry = input("Введите код инетересующей вас валюты:\n").upper()
    valute_input = input("Сколько конвентировать рубли?:\n")
    currency_output(valute_inquiry, valute_input)
